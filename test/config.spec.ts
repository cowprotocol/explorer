...
describe('Test config defaults', () => {
  ...
  it('disabledTokens', () => {
    ...
    const { [Network.MAINNET]: disabledOnMainnet, [Network.GNOSIS_CHAIN]: disabledOnXdai } = CONFIG.disabledTokens

    if (disabledOnMainnet.length) expect(disabledOnMainnet).toEqual(disabledTokensArray)
    if (disabledOnXdai.length) expect(disabledOnXdai).toEqual(disabledTokensArray)
  })
